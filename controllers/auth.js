 ...
 exports.login = (req, res) => {
    ...
    User.findOne({ email })
      .then(user => {
        console.log(user)
        if (!user) {
          return res.status(401).json({ message: 'User could not find. Please sign up first!' });
        }
   return bcrypt.compare(password, user.password)
      ...
      .catch(error => {
        res.status(500).json({ message: 'Internal server error. Please try again in a while.' });
      });
};
...
exports.signUp = async (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(422).json({ message: "Invalid credentials! Please fix inputs" });
  };
